...
// additional code to export CSV
	_csvDataExport: function(exportURL) {
			//call again for data - exportURL
							let v4URL = exportURL.substring(3);
							$.ajax({
								url: "/EZYCOMMERCE_SERVER" + v4URL,
								type: "GET",
								async: false,
								context: this,
								contentType: "application/json",
			...
			//
										let gridTable = this.getView().byId("idBRSTableMasterVariantReponsiveDataTable");
										let oModel = gridTable.getModel();
										let modelDataTemp = {};
										modelDataTemp.results = oSucc.value;
										var dataBefore = oSucc.value;
										var dataAfter = [];
										var dataArray = {} ;
										for (let i = 0; i < dataBefore.length; i++) {
										  dataArray = dataBefore[i];
										  //let oDate = new Date(dataArray.Date);
										  dataArray.dateString = dataArray.Date.toString();
										  dataArray.dateString = dataArray.dateString.substring(0, 10);
										  dataAfter.push(dataArray);
										}
										modelDataTemp.resultsNew = dataAfter ;
										var modelData = modelDataTemp;
										// JSON MODEL!!!!
